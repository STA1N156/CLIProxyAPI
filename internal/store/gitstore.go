...
func (s *GitTokenStore) recoverRepositoryLocked(repoDir string, authMethod []client.Option, sourceRepo *git.Repository, baselineTree *object.Tree, dirtyPaths map[string]struct{}) (errRecovery error) {
	...
	if baselineTree == nil {
		inspectedTree, inspectedDirtyPaths, errInspect := inspectRecoveryBaseline(sourceRepo)
		if errInspect != nil {
	...
	if errClose := sourceRepo.Close(); errClose != nil {
		return fmt.Errorf("close corrupt repository: %w", errClose)
	}
	if errClose := clonedRepo.Close(); errClose != nil {
		return fmt.Errorf("close recovered repository: %w", errClose)
	}
